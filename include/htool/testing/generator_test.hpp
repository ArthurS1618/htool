...
class GeneratorTestWithPermutation : public VirtualGeneratorInUserNumbering<CoefficientPrecision> {
  protected:
...
class GeneratorTestDouble : public GeneratorTestWithPermutation<double> {
  public:
    ...
    double get_coef(const int &i, const int &j) const override {
        return 1. / (1. + 4 * M_PI * std::sqrt(std::inner_product(p1.begin() + this->space_dim * i, this->p1.begin() + this->space_dim * i + this->space_dim, p2.begin() + this->space_dim * j, double(0), std::plus<double>(), [](double u, double v) { return (u - v) * (u - v); })));
    }
};
...
template <typename T>
class GeneratorInUserNumberingFromMatrix : public VirtualGeneratorInUserNumbering<T> {
  public:
    const Matrix<T> &A;
...
// class GeneratorFromMatrix : public GeneratorWithPermutation<T> {

//   public:
//     explicit GeneratorFromMatrix(const Matrix<T> &A0, const std::vector<int> &target_permutation, const std::vector<int> &source_permutation) : GeneratorWithPermutation<T>(GeneratorInUserNumberingFromMatrix(A0), target_permutation.data(), source_permutation.data()) {}

//   protected:
//     class GeneratorInUserNumberingFromMatrix : VirtualGeneratorInUserNumbering<T> {
//       public:
//         const Matrix<T> &A;
...
template <typename T>
class LocalGeneratorInUserNumberingFromMatrix : public VirtualGeneratorInUserNumbering<T> {
    const Matrix<T> &m_A;
...
// class LocalGeneratorFromMatrix : public GeneratorWithPermutation<T> {
//     const std::vector<int> &m_target_local_to_global_numbering;
//     const std::vector<int> &m_source_local_to_global_numbering;

//   public:
//     explicit LocalGeneratorFromMatrix(const Matrix<T> &A, const std::vector<int> &target_permutation, const std::vector<int> &source_permutation, const std::vector<int> &target_local_to_global_numbering, const std::vector<int> &source_local_to_global_numbering) : GeneratorWithPermutation<T>(LocalGeneratorInUserNumberingFromMatrix(A), target_permutation.data(), source_permutation.data()), m_target_local_to_global_numbering(target_local_to_global_numbering), m_source_local_to_global_numbering(source_local_to_global_numbering) {}

//   private:
//     class LocalGeneratorInUserNumberingFromMatrix : VirtualGeneratorInUserNumbering<T> {
//       public:
//         const Matrix<T> &m_A;
