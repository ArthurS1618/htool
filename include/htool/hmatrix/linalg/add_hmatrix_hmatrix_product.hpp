...
template <typename CoefficientPrecision, typename CoordinatePrecision = CoefficientPrecision>
void add_hmatrix_hmatrix_product_csr(char transa, char transb, CoefficientPrecision alpha, const HMatrix<CoefficientPrecision, CoordinatePrecision> &A, const HMatrix<CoefficientPrecision, CoordinatePrecision> &B, CoefficientPrecision beta, LowRankMatrix<CoefficientPrecision, CoordinatePrecision> &C) {
    if (beta != CoefficientPrecision(1)) {
        scale(beta, C);
    }
    ...
    if (A.is_hierarchical() and B.is_hierarchical()) {
        ...
        for (auto &output_cluster_child : output_clusters) {
            for (auto &input_cluster_child : input_clusters) {
                ...
                for (auto &middle_cluster_child : middle_clusters) {
                    const HMatrix<CoefficientPrecision, CoordinatePrecision> *A_child = (transa == 'N') ? A.get_sub_hmatrix(*output_cluster_child, *middle_cluster_child) : A.get_sub_hmatrix(*middle_cluster_child, *output_cluster_child);
                    const HMatrix<CoefficientPrecision, CoordinatePrecision> *B_child = (transb == 'N') ? B.get_sub_hmatrix(*middle_cluster_child, *input_cluster_child) : B.get_sub_hmatrix(*input_cluster_child, *middle_cluster_child);
                    if (!((A_child->is_low_rank() && A_child->get_low_rank_data()->get_U().nb_cols() == 0) || (B_child->is_low_rank() && B_child->get_low_rank_data()->get_U().nb_cols()))) {
                        add_hmatrix_hmatrix_product(transa, transb, alpha, *A_child, *B_child, CoefficientPrecision(1), low_rank_matrices.back());
                    }
                }
            }
        ...
        for (auto &output_cluster_child : output_clusters) {
            for (auto &input_cluster_child : input_clusters) {
                if (C.get_U().nb_cols() == 0) {
                    C.get_U().resize(output_cluster_child->get_size(), 0);
                    C.get_V().resize(0, input_cluster_child->get_size());
                }
                if (low_rank_matrices[index].get_U().nb_cols() > 0) {
                    add_lrmat_lrmat(low_rank_matrices[index], *output_cluster_child, *input_cluster_child, C, output_cluster, input_cluster);
                }
                index++;
            }
...
void add_hmatrix_hmatrix_product(char transa, char transb, CoefficientPrecision alpha, const HMatrix<CoefficientPrecision, CoordinatePrecision> &A, const HMatrix<CoefficientPrecision, CoordinatePrecision> &B, CoefficientPrecision beta, Matrix<CoefficientPrecision> &C) {
    ...
    if (A.get_target_cluster().get_rank() < 0 || A.get_source_cluster().get_rank() < 0 || B.get_target_cluster().get_rank() < 0 || B.get_source_cluster().get_rank() < 0) {
        ...
        add_hmatrix_matrix_product(transa, transb, alpha, A, B_dense, CoefficientPrecision(1), C); // It could be optimized, but it should not happen often...
        // std::vector<const Cluster<CoordinatePrecision> *> output_clusters, middle_clusters, input_clusters;
...
void add_hmatrix_hmatrix_product(char transa, char transb, CoefficientPrecision alpha, const HMatrix<CoefficientPrecision, CoordinatePrecision> &A, const HMatrix<CoefficientPrecision, CoordinatePrecision> &B, CoefficientPrecision beta, HMatrix<CoefficientPrecision, CoordinatePrecision> &C) {
    ...
    if (C.is_dense()) {
        ...
        if (A.is_hierarchical() and B.is_hierarchical()) {
                ...
                for (auto &input_cluster_child : input_clusters) {
                    for (auto &middle_cluster_child : middle_clusters) {
                        ...
                        if (A_child != nullptr && B_child != nullptr && C_child != nullptr) {
                            add_hmatrix_hmatrix_product(transa, transb, alpha, *A_child, *B_child, CoefficientPrecision(1), *C.get_child_or_this(*output_cluster_child, *input_cluster_child));
                        }
                    }
                }
            }
            ...
            if (A.is_low_rank() || B.is_low_rank()) {
                ...
                if (A.is_dense() and B.is_low_rank()) {
                    ...
                    add_hmatrix_lrmat_product(transa, transb, alpha, A, *B.get_low_rank_data(), beta, lrmat);
                    ...
                    add_lrmat_hmatrix_product(transa, transb, alpha, *A.get_low_rank_data(), B, beta, lrmat);
                }
                ...
                if (A.is_dense() and B.is_dense()) {
                    ...
                    add_matrix_hmatrix_product(transa, transb, alpha, *A.get_dense_data(), B, beta, lrmat);
                } else if (A.is_hierarchical() and B.is_dense()) {
                    add_hmatrix_matrix_product(transa, transb, alpha, A, *B.get_dense_data(), beta, lrmat);
                }

                add_lrmat_hmatrix(lrmat, C);
            }
        }
        // std::unique_ptr<Matrix<CoefficientPrecision>> C_dense_ptr = std::make_unique<Matrix<CoefficientPrecision>>(C.get_target_cluster().get_size(), C.get_source_cluster().get_size());
        // auto &C_dense                                             = *C_dense_ptr;
        ...
        //     }
        // }
        // C.set_dense_data(std::move(C_dense_ptr));
        // }
    }
}

template <typename CoefficientPrecision, typename CoordinatePrecision = CoefficientPrecision>
void add_hmatrix_hmatrix_product_csr(char transa, char transb, CoefficientPrecision alpha, const HMatrix<CoefficientPrecision, CoordinatePrecision> &A, const HMatrix<CoefficientPrecision, CoordinatePrecision> &B, CoefficientPrecision beta, HMatrix<CoefficientPrecision, CoordinatePrecision> &C) {

    if (C.is_dense()) {

        add_hmatrix_hmatrix_product(transa, transb, alpha, A, B, beta, *C.get_dense_data());
    } else if (C.is_low_rank()) {
        // if (C.get_low_rank_data()->get_U().nb_cols() == 0) {
        //     LowRankMatrix<CoefficientPrecision, CoordinatePrecision> lrmat(A.get_epsilon());
        //     C.set_low_rank_data(lrmat);
        // }
        if (!((A.is_low_rank() && A.get_low_rank_data()->get_U().nb_cols() == 0) || (B.is_low_rank() && B.get_low_rank_data()->get_U().nb_cols() == 0))) {
            add_hmatrix_hmatrix_product_csr(transa, transb, alpha, A, B, beta, *C.get_low_rank_data());
        }
    } else {
        if (A.is_hierarchical() and B.is_hierarchical()) {
            if (beta != CoefficientPrecision(1)) {
            ...
            for (auto &output_cluster_child : output_clusters) {
                for (auto &input_cluster_child : input_clusters) {
                    for (auto &middle_cluster_child : middle_clusters) {
                        ...
                        auto &&C_child = C.get_child_or_this(*output_cluster_child, *input_cluster_child);
                        if (A_child != nullptr && B_child != nullptr && C_child != nullptr) {
                            if (!((A_child->is_low_rank() && A_child->get_low_rank_data()->get_U().nb_cols() == 0) || (B_child->is_low_rank() && B_child->get_low_rank_data()->get_U().nb_cols() == 0))) {

                                add_hmatrix_hmatrix_product_csr(transa, transb, alpha, *A_child, *B_child, CoefficientPrecision(1), *C.get_child_or_this(*output_cluster_child, *input_cluster_child));
                            }
                        }
                    }
                }
            ...
            if (A.is_low_rank() || B.is_low_rank()) {
                ...
                if (A.is_dense() and B.is_low_rank()) {
                    if (B.get_low_rank_data()->get_U().nb_cols() > 0) {
                        add_matrix_lrmat_product(transa, transb, alpha, *A.get_dense_data(), *B.get_low_rank_data(), beta, lrmat);
                    }
                } else if (A.is_hierarchical() and B.is_low_rank()) {
                    if (B.get_low_rank_data()->get_U().nb_cols() > 0) {
                        add_hmatrix_lrmat_product(transa, transb, alpha, A, *B.get_low_rank_data(), beta, lrmat); /// ici
                    }
                } else if (A.is_low_rank() and B.is_low_rank()) {
                    if (A.get_low_rank_data()->get_U().nb_cols() > 0 && B.get_low_rank_data()->get_U().nb_cols() > 0) {
                        add_lrmat_lrmat_product(transa, transb, alpha, *A.get_low_rank_data(), *B.get_low_rank_data(), beta, lrmat);
                    }
                } else if (A.is_low_rank() and B.is_dense()) {
                    if (A.get_low_rank_data()->get_U().nb_cols() > 0) {
                        add_lrmat_matrix_product(transa, transb, alpha, *A.get_low_rank_data(), *B.get_dense_data(), beta, lrmat);
                    }
                } else if (A.is_low_rank() and B.is_hierarchical()) {
                    if (A.get_low_rank_data()->get_U().nb_cols() > 0) {

                        add_lrmat_hmatrix_product(transa, transb, alpha, *A.get_low_rank_data(), B, beta, lrmat); /// ici
                    }
                }
                add_lrmat_hmatrix(lrmat, C);
            } else {
...
// template <typename CoefficientPrecision, typename CoordinatePrecision = CoefficientPrecision>
// void add_symmetry_hmatrix_hmatrix_product(char side, char UPLO, CoefficientPrecision alpha, const HMatrix<CoefficientPrecision, CoordinatePrecision> &A, const HMatrix<CoefficientPrecision, CoordinatePrecision> &B, CoefficientPrecision beta, LowRankMatrix<CoefficientPrecision, CoordinatePrecision> &C) {
...
//             low_rank_matrices;
//         std::vector<const Cluster<CoordinatePrecision> *> A_target_clusters, middle_clusters, input_clusters;

//         const Cluster<CoordinatePrecision> &output_cluster = side == 'L' ? A.get_target_cluster() : B.get_target_cluster();
//         const Cluster<CoordinatePrecision> &middle_cluster = side == 'L' ? A.get_source_cluster() : A.get_source_cluster();
//         const Cluster<CoordinatePrecision> &input_cluster  = side == 'L' ? B.get_source_cluster() : A.get_source_cluster();
...
//                         A_child = A.get_sub_hmatrix(*output_cluster_child, *middle_cluster_child);
...
// template <typename CoefficientPrecision, typename CoordinatePrecision = CoefficientPrecision>
// void add_symmetry_hmatrix_hmatrix_product(char side, char UPLO, CoefficientPrecision alpha, const HMatrix<CoefficientPrecision, CoordinatePrecision> &A, const HMatrix<CoefficientPrecision, CoordinatePrecision> &B, CoefficientPrecision beta, Matrix<CoefficientPrecision> &C) {
//     if (side != 'L') {
//         htool::Logger::get_instance().log(LogLevel::ERROR, "Operation is not implemented for add_hmatrix_hmatrix_product_symmetric (side=" + std::string(1, side) + ")"); // LCOV_EXCL_LINE
//     }
//     if (beta != CoefficientPrecision(1)) {
...
//         Matrix<CoefficientPrecision> B_dense(B.get_target_cluster().get_size(), B.get_source_cluster().get_size());
//         copy_to_dense(B, B_dense.data());
//         add_symmetry_hmatrix_matrix_product(side, UPLO, alpha, A, B_dense, CoefficientPrecision(1), C);
//     } else {
//         add_hmatrix_hmatrix_product(transa, transb, alpha, A, B, CoefficientPrecision(1), C);
//     }
// }
