    ...
    if (rank != 0) {
        ...
        if (beta == CoefficientPrecision(0) || C.rank_of() == 0) {
            if (transa == 'N') {
                ...
                U_C = U_A;
                add_matrix_hmatrix_product<CoefficientPrecision>(transa, transb, alpha, V_A, B, 0, V_C);
            } else {
                ...
                add_matrix_hmatrix_product<CoefficientPrecision>(transa, transb, alpha, U_A, B, 0, V_C);
            }
        } else {
            ...
            if (transa == 'N') {
                ...
                add_matrix_hmatrix_product<CoefficientPrecision>(transa, transb, alpha, V_A, B, 0, VB);
            } else {
                ...
                // Compute VB=V_B*B
                VB.resize(V_A.nb_rows(), transb == 'N' ? B.nb_cols() : B.nb_rows());
                add_matrix_hmatrix_product<CoefficientPrecision>(transa, transb, alpha, U_A, B, 0, VB);
            }
...
template <typename CoefficientPrecision, typename CoordinatePrecision = CoefficientPrecision>
void add_lrmat_hmatrix_product(char transa, char transb, CoefficientPrecision alpha, const LowRankMatrix<CoefficientPrecision, CoordinatePrecision> &A, const HMatrix<CoefficientPrecision, CoordinatePrecision> &B, CoefficientPrecision beta, HMatrix<CoefficientPrecision, CoordinatePrecision> &C) {
    if (beta != CoefficientPrecision(1)) {
        scale(beta, C);
    }

    LowRankMatrix<CoefficientPrecision, CoordinatePrecision> lrmat(A.get_epsilon());
    add_lrmat_hmatrix_product(transa, transb, alpha, A, B, CoefficientPrecision(1), lrmat);
