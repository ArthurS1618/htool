...
void triangular_hmatrix_hmatrix_solve(char side, char UPLO, char transa, char diag, CoefficientPrecision alpha, const HMatrix<CoefficientPrecision, CoordinatePrecision> &A, HMatrix<CoefficientPrecision, CoordinatePrecision> &B) {
    ...
    if (A.is_hierarchical() and B.is_hierarchical()) {
                ...
                for (auto &input_cluster_child : input_clusters) {
                    ...
                    for (auto &middle_cluster_child : middle_clusters) {
                        ...
                        if (*output_cluster_child == *middle_cluster_child) {
                            triangular_hmatrix_hmatrix_solve(side, UPLO, transa, diag, CoefficientPrecision(1), *A_child, *B_child_to_modify);
                            ...
                            add_hmatrix_hmatrix_product(transa, 'N', CoefficientPrecision(-1), *A_child, *B_child, CoefficientPrecision(1), *B_child_to_modify);
                        }
                    }
                }
            }
                ...
                for (auto output_it = output_clusters.begin(); output_it != output_clusters.end(); ++output_it) {
                    ...
                    for (auto middle_it = middle_clusters.begin(); middle_it != middle_clusters.end(); ++middle_it) {
                        ...
                        if (*input_cluster_child == *middle_cluster_child) {
                            ...
                            add_hmatrix_hmatrix_product('N', transa, CoefficientPrecision(-1), *B_child, *A_child, CoefficientPrecision(1), *B_child_to_modify);
                        }
                    }
                }
            }
        ...
        if (A.is_dense()) {
            ...
            if (B.is_low_rank()) {
                triangular_hmatrix_lrmat_solve(side, UPLO, transa, diag, CoefficientPrecision(1), A, *B.get_low_rank_data());
            } else if (B.is_dense()) {
                triangular_hmatrix_matrix_solve(side, UPLO, transa, diag, CoefficientPrecision(1), A, *B.get_dense_data());
            }
        }
    }
}

///////////// je rajoute ca pour les csr

template <typename CoefficientPrecision, typename CoordinatePrecision>
void triangular_hmatrix_hmatrix_solve_csr(char side, char UPLO, char transa, char diag, CoefficientPrecision alpha, const HMatrix<CoefficientPrecision, CoordinatePrecision> &A, HMatrix<CoefficientPrecision, CoordinatePrecision> &B) {
    if (alpha != CoefficientPrecision(1)) {
        scale(alpha, B);
    }
    if (A.is_hierarchical() and B.is_hierarchical()) {
        ...
        if ((UPLO == 'L' && transa == 'N' && side == 'L') || (UPLO == 'U' && transa != 'N' && side == 'L')) {
            for (auto &output_cluster_child : output_clusters) {
                for (auto &input_cluster_child : input_clusters) {
                    ...
                    for (auto &middle_cluster_child : middle_clusters) {
                        const HMatrix<CoefficientPrecision, CoordinatePrecision> *A_child = transa == 'N' ? A.get_sub_hmatrix(*output_cluster_child, *middle_cluster_child) : A.get_sub_hmatrix(*middle_cluster_child, *output_cluster_child);
                        if (!(A_child->is_low_rank() && A_child->get_low_rank_data()->get_U().nb_cols() == 0)) {

                            if (*output_cluster_child == *middle_cluster_child) {
                                triangular_hmatrix_hmatrix_solve_csr(side, UPLO, transa, diag, CoefficientPrecision(1), *A_child, *B_child_to_modify);
                            } else if (output_cluster_child->get_offset() > middle_cluster_child->get_offset()) {

                                const HMatrix<CoefficientPrecision, CoordinatePrecision> *B_child = B.get_sub_hmatrix(*middle_cluster_child, *input_cluster_child);
                                if (!(B_child->is_low_rank() && B_child->get_low_rank_data()->get_U().nb_cols() == 0)) {

                                    add_hmatrix_hmatrix_product_csr(transa, 'N', CoefficientPrecision(-1), *A_child, *B_child, CoefficientPrecision(1), *B_child_to_modify);
                                }
                            }
                        }
                    }
                        ...
                        if (!(A_child->is_low_rank() && A_child->get_low_rank_data()->get_U().nb_cols() == 0)) {
                            if (*input_cluster_child == *middle_cluster_child) {
                                ...
                                if (!(B_child->is_low_rank() && B_child->get_low_rank_data()->get_U().nb_cols() == 0)) {
                                    ...
                                    //     Matrix<CoefficientPrecision> Uzeros(output_cluster_child->get_size(), 0);
                                    //     Matrix<CoefficientPrecision> Vzeros(0, input_cluster_child->get_size());
                                    //     LowRankMatrix<CoefficientPrecision, CoordinatePrecision> lr(Uzeros, Vzeros);
                                    //     B_child_to_modify->set_low_rank_data(lr);
                                    // } else {
                                    add_hmatrix_hmatrix_product_csr('N', transa, CoefficientPrecision(-1), *B_child, *A_child, CoefficientPrecision(1), *B_child_to_modify);
                                    // }
                                }
                            }
                        }
                ...
                for (auto output_it = output_clusters.rbegin(); output_it != output_clusters.rend(); ++output_it) {
                    ...
                    for (auto middle_it = middle_clusters.rbegin(); middle_it != middle_clusters.rend(); ++middle_it) {
                        ...
                        if (*input_cluster_child == *middle_cluster_child) {
                            ...
                            //     Matrix<CoefficientPrecision> Uzero(output_cluster_child->get_size(), 0);
                            //     std::cout << "!!!!!!!!!!!!!!!!!!!!!!!" << std::endl;
                            //     Matrix<CoefficientPrecision> Vzero(0, input_cluster_child->get_size());
                            //     LowRankMatrix<CoefficientPrecision, CoordinatePrecision> lr(Uzero, Vzero);
                            //     B_child_to_modify->set_low_rank_data(lr);
                            // } else {
                            add_hmatrix_hmatrix_product('N', transa, CoefficientPrecision(-1), *B_child, *A_child, CoefficientPrecision(1), *B_child_to_modify);
                            // }
        ...
        if (A.is_dense()) {
            ...
            if (B.is_low_rank() && B.get_low_rank_data()->get_U().nb_cols() > 0) {
                triangular_hmatrix_lrmat_solve(side, UPLO, transa, diag, CoefficientPrecision(1), A, *B.get_low_rank_data());
            } else if (B.is_dense()) {
                triangular_hmatrix_matrix_solve(side, UPLO, transa, diag, CoefficientPrecision(1), A, *B.get_dense_data());
            }
