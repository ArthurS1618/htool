...
void triangular_hmatrix_matrix_solve_row_major(char side, char UPLO, char transa, char diag, CoefficientPrecision alpha, const HMatrix<CoefficientPrecision, CoordinatePrecision> &A, Matrix<CoefficientPrecision> &Bt) {
            ...
            for (auto &output_cluster_child : output_clusters) {
                ...
                for (auto &input_cluster_child : input_clusters) {
                    ...
                    if (*output_cluster_child == *input_cluster_child) {
                        triangular_hmatrix_matrix_solve_row_major(side, UPLO, transa, diag, CoefficientPrecision(1), *A_child, current_Bt_to_modify);
                        ...
                        add_hmatrix_matrix_product_row_major(transa, 'N', CoefficientPrecision(-1), *A_child, current_Bt.data(), CoefficientPrecision(1), current_Bt_to_modify.data(), current_Bt.nb_rows());
                    }
                }
            }
        } // Backward
...
template <typename CoefficientPrecision, typename CoordinatePrecision>
void triangular_hmatrix_matrix_solve(char side, char UPLO, char transa, char diag, CoefficientPrecision alpha, const HMatrix<CoefficientPrecision, CoordinatePrecision> &A, Matrix<CoefficientPrecision> &B) {
    if (side == 'L') {
        Matrix<CoefficientPrecision> transposed_B(B.nb_cols(), B.nb_rows());
        transpose(B, transposed_B);
        triangular_hmatrix_matrix_solve_row_major(side, UPLO, transa, diag, alpha, A, transposed_B);
