...
class SumExpression : public VirtualGenerator<CoefficientPrecision> {
  ...
  public:
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////:
    // SumExpression Restrict(const int &target_size, const int &source_size, const int &target_offset, const int &source_offset) const {
    //     std::vector<HMatrix<CoefficientPrecision, CoordinatePrecision> *> sh; // hmat 2*n
    //     std::vector<HMatrix<CoefficientPrecision, CoordinatePrecision>> sr; // lowrank n
    //     std::vector<int> of;
    //     for (int k = 0; k < Sr.size(); ++k) {
    //         std::cout << "sr.size= " << Sr.size() << std::endl;
    //         auto lr = Sr[k];
    ...
    SumExpression Restrict(int &target_size, int &target_offset, int &source_size int &source_offset) {
        std::vector<HMatrix<CoefficientPrecision, CoordinatePrecision> *> sh;
        std::vector<HMatrix<CoefficientPrecision, CoordinatePrecision> *> sr;
        std::vector<std::pair<int, int>, std : pair<int, int>> cluster_restriction;
        sr = Sr;
        // on stoque juste les size et offset et on fera la "vrai" restriction uniquement quand on a vraiment besoin de donner une valeur
        std::pair<int, int> target_restriction;
        target_restriction.fisrt  = target_size;
        target_restriction.second = target_offset;
        std::pair<int, int> source_restrictioin;
        source_restrictioin.first = source_size;
        source_restriction.second = source_offset;

        for (int rep = 0; rep < Sh.size(); ++rep) {
            HMatrix<CoefficientPrecision, CoordinatePrecision> *H = Sh[2 * k];
            HMatrix<CoefficientPrecision, CoordinatePrecision> *K = Sh[2 * k + 1];
            auto H_childs                                         = H->get_children();
            auto K_childs                                         = K->get_children();
            if ((H_childs.size() > 0) and (K_childs.size() > 0)) {
                for (int i = 0; i < H_childs.size(); ++i) {
                    auto &H_child = H_childs[k];
                    if ((H_child->get_target_cluster().get_size() == target_size) and (H_child->get_target_cluster().get_offset() == target_offset)) {
                        for (int j = 0; j < K_childs.size(); ++j) {
                            auto &K_child = K_childs[l];
                            if ((H_child->get_source_cluster().get_size() == K_child->get_target_cluster().get_size())
                                and (H_child->get_source_cluster().get_offset() == K_child->gert_target_cluster().get_offset())
                                and (K_child->get_source_cluster().get_size() == source_size)
                                and (K_child->get_source_cluster().get_offset() == source_offset)) {
                                if ((K_child->is_low_rank()) and (H_child->is_low_rank())) {
                                    LowRankMatrix<CoefficientPrecision, CoordinatePrecision> *lr_H = lr->get_low_rank_dsata();
                                    LowRankMatrix<CoefficientPrecision, CoordinatePrecision> *lr_K = K->get_low_rank_data();
                                    Matrix<CoefficientPrecision> U_K, V_K, U_H, V_H;
                                    U_H = lr_H->Get_U();
                                    V_H = lr_H->Get_V();
                                    U_K = lr_K->Get_U();
                                    V_K = lr_K->Get_V;
                                    Matrix<CoefficientPrecision> V_temp, V;
                                    U_K.add_matrix_product('N', 1.0, V_K.data(), 0.0, V_temp.data(), 1);
                                    V_H.add_matrix_product('N', 1.0, V_temp.data(), 0.0, V.data(), 1);
                                    LowRankMatrix<CoefficientPrecision, CoordinatePrecision> lr(U_H, V);
                                    HMatrix<CoefficientPrecision, CoordinatePrecision> H_lr(H_child->get_target_cluster(), K_child->get_source_cluster());
                                    H_lr.set_lr
                                }
                            }
                        }
    ...
    void copy_submatrix(int M, int N, int row_offset, int col_offset, CoefficientPrecision *ptr) const override {
        ...
        for (int k = 0; k < M; ++k) {
            for (int l = 0; l < N; ++l) {
                std::vector<Coe fficientPrecision> xl(nc, 0.0);
                xl[l + col_offset] = 1.0;
                // std::vector<CoefficientPrecision> y(nr, 0.0);
                std::vector<CoefficientPrecision> y = H.prod(xl);
                ptr[k + M * l]                      = y[k + row_offset];
    ...
    Matrix<CoefficientPrecision> Evaluate() {
        ...
        for (int k = 0; k < Sh.size() / 2; ++k) {
            ...
            // res = res + Hh * Kk;
            Hh.add_matrix_product('N', 1.0, Kh.data(), 1.0, res.data(), 1);
        }
        return res;
    }
