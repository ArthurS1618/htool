...
void hmat_hmat(SumExpression_fast<double, double> &sumexpr, HMatrix<double, double> *res) {
    ...
    bool flag_size_min = ((res->get_target_cluster().get_children().size()) == 0);
    if (flag_adm) {
        ...
        if ((lr.rank_of() == -1)) {
            if (res->get_target_cluster().get_children().size() > 0) {
                for (auto &t_child : res->get_target_cluster().get_children()) {
                    for (auto &s_child : res->get_source_cluster().get_children()) {
                        auto s_restr   = sumexpr.restrict_ACA(*t_child, *s_child);
            ...
            std::cout << "--------------->affect lr hmat  ok" << std::endl;
        }
    } else {
        if (res->get_target_cluster().get_children().size() == 0) {
            std::cout << "affect dense  hmat " << std::endl;
            ...
            std::cout << "---------------k--->affect dense  hmat ok" << std::endl;
        } else {
            for (auto &t_child : res->get_target_cluster().get_children()) {
                for (auto &s_child : res->get_source_cluster().get_children()) {
                    auto res_child        = res->add_child(t_child.get(), s_child.get());
                    auto sumexpr_restrict = sumexpr.restrict_ACA(*t_child, *s_child);
...
int test_sum_expression(int size, int rank, htool::underlying_type<T> epsilon) {
    ...
    if (res.size() > 1) {
        ...
        auto restr = sum_expr.restrict_ACA(*t1, *t2);
        ...
        auto restr1 = restr.restrict_ACA(*t11, *t22);
        ...
        auto restr2 = restr1.restrict_ACA(*t111, *t222);
        ...
        auto restr3 = restr2.restrict_ACA(*t1111, *t2222);
