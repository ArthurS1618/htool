...
bool test_hmatrix_matrix_product(const TestCaseProduct<T, GeneratorTestType> &test_case, bool use_local_cluster, htool::underlying_type<T> epsilon, htool::underlying_type<T> margin) {
    ...
    matrix_test = C_dense;
    add_hmatrix_matrix_product(transa, transb, alpha, root_hmatrix, B_dense, beta, matrix_test);
    ...
    if (transb == 'N') {
        ...
        openmp_add_hmatrix_matrix_product_row_major(transa, transb, alpha, root_hmatrix, transposed_B_dense.data(), beta, matrix_test.data(), C_dense.nb_cols());
        ...
        matrix_test = transposed_C_dense;
        sequential_add_hmatrix_matrix_product_row_major(transa, transb, alpha, root_hmatrix, transposed_B_dense.data(), beta, matrix_test.data(), C_dense.nb_cols());
        error    = normFrob(transposed_matrix_result_w_matrix_sum - matrix_test) / normFrob(transposed_matrix_result_w_matrix_sum);
        is_error = is_error || !(error < epsilon);
    ...
    add_hmatrix_matrix_product(transa, transb, alpha, root_hmatrix, B_dense, T(0), lrmat_test);
    ...
    lrmat_test = C_auto_approximation;
    add_hmatrix_matrix_product(transa, transb, alpha, root_hmatrix, B_dense, beta, lrmat_test);
    dense_lrmat_test.resize(lrmat_test.get_U().nb_rows(), lrmat_test.get_V().nb_cols());
    lrmat_test.copy_to_dense(dense_lrmat_test.data());
...
bool test_symmetric_hmatrix_matrix_product(const TestCaseSymmetricProduct<T, GeneratorTestType> &test_case, htool::underlying_type<T> epsilon, htool::underlying_type<T>) {
    ...
    // matrix_test = C_dense;
    // add_symmetric_hmatrix_matrix_product(side, UPLO, alpha, root_hmatrix, B_dense, beta, matrix_test);
    // error    = normFrob(matrix_result_w_matrix_sum - matrix_test) / normFrob(matrix_result_w_matrix_sum);
    // is_error = is_error || !(error < epsilon);
    ...
    if (side == 'L') {
        ...
        matrix_test = transposed_C_dense;
        sequential_add_symmetric_hmatrix_matrix_product_row_major(side, UPLO, alpha, root_hmatrix, transposed_B_dense.data(), beta, matrix_test.data(), C_dense.nb_cols());
        error    = normFrob(transposed_matrix_result_w_matrix_sum - matrix_test) / normFrob(transposed_matrix_result_w_matrix_sum);
        is_error = is_error || !(error < epsilon);
    ...
    // // Product
    // lrmat_test = C_auto_approximation;
    // add_symmetric_hmatrix_matrix_product(side, UPLO, alpha, root_hmatrix, B_dense, T(0), lrmat_test);
    // dense_lrmat_test.resize(lrmat_test.get_U().nb_rows(), lrmat_test.get_V().nb_cols());
    // lrmat_test.copy_to_dense(dense_lrmat_test.data());
    // error    = normFrob(matrix_result_wo_sum - dense_lrmat_test) / normFrob(matrix_result_wo_sum);
    // is_error = is_error || !(error < std::max(epsilon, lrmat_tolerance));
    ...
    // lrmat_test = C_auto_approximation;
    // add_symmetric_hmatrix_matrix_product(transa, transb, alpha, root_hmatrix, B_dense, beta, lrmat_test);
    // dense_lrmat_test.resize(lrmat_test.get_U().nb_rows(), lrmat_test.get_V().nb_cols());
    // lrmat_test.copy_to_dense(dense_lrmat_test.data());
    // error    = normFrob(matrix_result_w_lrmat_sum - dense_lrmat_test) / normFrob(matrix_result_w_lrmat_sum);
    // is_error = is_error || !(error < std::max(epsilon, lrmat_tolerance) * margin);
    // cout << "> Errors on a symmetric_hmatrix matrix product to lrmat with sum: " << error << endl;
