...
bool test_hmatrix_triangular_solve(char side, char transa, int n1, int n2, htool::underlying_type<T> epsilon, htool::underlying_type<T> margin) {
    ...
    if (side == 'L') {
        ...
        add_hmatrix_hmatrix_product(transa, 'N', T(1) / alpha, UA, X, T(0), UB);
        add_hmatrix_hmatrix_product(transa, 'N', T(1) / alpha, LA, X, T(0), LB);
        ...
        add_hmatrix_hmatrix_product('N', transa, T(1) / alpha, X, UA, T(0), UB);
        add_hmatrix_hmatrix_product('N', transa, T(1) / alpha, X, LA, T(0), LB);
    }
    ...
    triangular_hmatrix_matrix_solve(side, 'L', transa, 'N', alpha, LA, matrix_test);
    ...
    triangular_hmatrix_matrix_solve(side, 'U', transa, 'N', alpha, UA, matrix_test);
    error    = normFrob(X_dense - matrix_test) / normFrob(X_dense);
    is_error = is_error || !(error < epsilon * margin);
    cout << "> Errors on upper triangular hmatrix matrix solve: " << error << endl;

    lrmat_test = LB_lrmat;
    triangular_hmatrix_lrmat_solve(side, 'L', transa, 'N', alpha, LA, lrmat_test);
    ...
    lrmat_test = UB_lrmat;
    triangular_hmatrix_lrmat_solve(side, 'U', transa, 'N', alpha, UA, lrmat_test);
    dense_lrmat_test.resize(lrmat_test.get_U().nb_rows(), lrmat_test.get_V().nb_cols());
    lrmat_test.copy_to_dense(dense_lrmat_test.data());
    error    = normFrob(dense_X_lrmat - dense_lrmat_test) / normFrob(dense_X_lrmat);
    is_error = is_error || !(error < epsilon * margin);
    ...
    hmatrix_test = LB;
    triangular_hmatrix_hmatrix_solve(side, 'L', transa, 'N', alpha, LA, hmatrix_test);
    ...
    hmatrix_test = UB;
    triangular_hmatrix_hmatrix_solve(side, 'U', transa, 'N', alpha, UA, hmatrix_test);
    copy_to_dense(hmatrix_test, densified_hmatrix_test.data());
    error    = normFrob(X_dense - densified_hmatrix_test) / normFrob(X_dense);
    is_error = is_error || !(error < epsilon * margin);
