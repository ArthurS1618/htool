...
bool test_matrix_lrmat_product(const TestCaseProduct<T, GeneratorTestType> &test_case, htool::underlying_type<T> epsilon, htool::underlying_type<T> additional_compression_tolerance, htool::underlying_type<T> additional_lrmat_sum_tolerance) {
    bool is_error = false;
    ...
    matrix_test = C_dense;
    add_matrix_lrmat_product(transa, transb, alpha, A_dense, B_fixed_approximation, beta, matrix_test);
    ...
    add_matrix_lrmat_product(transa, transb, alpha, A_dense, B_fixed_approximation, T(0), lrmat_test);
    ...
    lrmat_test = C_fixed_approximation;
    add_matrix_lrmat_product(transa, transb, alpha, A_dense, B_fixed_approximation, beta, lrmat_test);
    dense_lrmat_test.resize(lrmat_test.get_U().nb_rows(), lrmat_test.get_V().nb_cols());
    lrmat_test.copy_to_dense(dense_lrmat_test.data());
